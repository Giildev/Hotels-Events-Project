...
{
    global $indexes;
    $realdir = SYSBASE.$dir;      //toma la direccion url (/home/gilberto/public_html/Staycation/admin/modules)

    $rep = opendir($realdir) or die("Error directory opening: ".$realdir);

      while($entry = @readdir($rep)){     //$entry = booking -- @readdir($rep)=" . "

        if(is_dir($realdir."/".$entry) && $entry != "." && $entry != ".." && substr($entry, 0, 1) != ".")
            $modules = getModules($dir."/".$entry, $modules);
        else{
            if(is_file($realdir."/".$entry) && $entry == "config.xml"){ // $entry = config.xml
