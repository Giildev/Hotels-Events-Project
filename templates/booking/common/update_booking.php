    ...
    if($result_service !== false){
        foreach($result_service as $i => $row){
            ...
            if($qty > 0){
                ...
                if(ENABLE_BOOKING_REQUESTS == 0) $vat_total += $price-($price/($vat_rate/100+1));
            }
        }
    }
    if($total_services > 0){
...
if(ENABLE_BOOKING_REQUESTS == 0) $response['html'] = formatPrice($total*CURRENCY_RATE)."<br><small>".formatPrice($vat_total*CURRENCY_RATE)."</small>";
