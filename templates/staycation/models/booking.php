...
if(isset($_POST['book']) || (ENABLE_BOOKING_REQUESTS == 1 && isset($_POST['request']))){
    ...
    if(isset($_POST['book'])){
        ...
        $_SESSION['book']['total'] = $_SESSION['book']['amount']+$_SESSION['book']['tourist_tax'];
...
if(count($field_notice) == 0){
    ...
    if($num_nights <= 0) $msg_error .= $texts['NO_AVAILABILITY'];
        ...
        $res_hotel = array();
        $query_rate = "
            SELECT DISTINCT id_hotel, id_room, start_date, end_date, ra.price, child_price, discount, type, people, price_sup, fixed_sup, vat_rate, min_stay, day_start, day_end
            FROM pm_rate as ra, pm_room as ro
            WHERE
                id_room = ro.id
                AND ro.checked = 1
                AND (end_lock < ".$from_time." OR
                    start_lock > ".$to_time.")
        ...
        if($result_room_rate !== false){
            foreach($result_room_rate as $i => $row){
                ...
                if(($day_start == 0 || $from_n == $day_start) && ($day_end == 0 || $to_n == $day_end) && $num_nights >= $min_stay){
                    ...
                    XOR ($type == "night")){
                        ...
                        $dst = false;
                        $step = 86400;
                        for($date = $start; $date < ($start+(86400*$n_period)); $date += $step){
                            $cur_dst = date("I", $date);
                            if($dst != $cur_dst){
                                if($cur_dst == 0){
                                    $date += 3600;
            ...
            foreach($res_hotel as $id_hotel => $hotel){
                foreach($hotel as $id_room => $result){
                    if(!isset($result['amount']) || $result['amount'] == 0 || $result['total_nights'] != $num_nights) unset($res_hotel[$id_hotel][$id_room]);
...
<section id="page">
    ...
    <div id="content" class="pt30 pb30">
        ...
        $result_hotel_rate = $db->prepare("SELECT DISTINCT(ra.price), type FROM pm_rate as ra, pm_room as ro WHERE ro.id = id_room AND id_hotel = :id_hotel AND ra.price IN(SELECT MIN(ra.price) FROM pm_rate as ra, pm_room as ro WHERE ro.id = id_room AND id_hotel = :id_hotel)");
        ...
        $query_hotel = "SELECT * FROM pm_hotel WHERE checked = 1 AND lang = ".LANG_ID." ORDER BY";
                ...
                <div class="container boxed mb20 booking-result">
                    <div class="row">
                        <div class="col-sm-4 mb10">
                            ...
                            if($result_hotel_file !== false && $db->last_row_count() == 1){
                        ...
                        <div class="col-sm-4 col-md-5">
                            ...
                            <div class="clearfix mt10">
                                ...
                                if($result_hotel_facilities !== false && $db->last_row_count() > 0){
                                    ...
                                    <span class="facility-icon">
                                        <a href="<?php echo DOCBASE.$pages[9]['alias']."/".text_format($hotel_alias); ?>" title="<?php echo $texts['READMORE']; ?>" class="tips">...</a>
                                    </span>
                                    <?php
                                } ?>
                            </div>
                        ...
                        <div class="col-sm-4 col-md-3 text-center sep">
                            <div class="price text-primary">
                                ...
                                if($result_hotel_rate !== false && $db->last_row_count() == 1){
                            ...
                            <a class="btn btn-primary mt10 btn-block" href="<?php echo DOCBASE.$pages[9]['alias']."/".text_format($hotel_alias); ?>">
                            ...
                            <div class="boxed">
                                ...
                                if($result_room !== false){
                                    foreach($result_room as $row){
                                        ...
                                        if($result_room_rate !== false && $db->last_row_count() == 1){
                                        ...
                                        if(!isset($res_hotel[$id_hotel][$id_room]) || ($num_adults+$num_children > $max_people) || ($num_adults+$num_children < $min_people)){
                                            $amount = $min_price;
                                            $full_price = 0;
                                        }else{
                                                ...
                                                <div class="col-lg-2 col-md-2 col-sm-3 text-center sep">
                                                    ...
                                                    <p class="lead pt10">
                                                        ...
                                                        if(!isset($res_hotel[$id_hotel][$id_room])){ ?>
                                                            ...
                                                            <?php
                                                        }elseif($res_hotel[$id_hotel][$id_room]['min_stay'] > 0 && $num_nights < $res_hotel[$id_hotel][$id_room]['min_stay']){ ?>
                                                            <button class="btn btn-danger btn-block" disabled="disabled"><i class="fa fa-warning"></i> <small><?php echo $texts['MIN_NIGHTS']; ?> : <?php echo $res_hotel[$id_hotel][$id_room]['min_stay']; ?></small></button>
                                                            <?php
                                                        }else{ ?>
                                                        ...
                                                        <a class="btn btn-primary mt10 btn-block popup-modal" href="#room-<?php echo $id_room; ?>">
                                                            <i class="fa fa-plus-circle"></i>
                                                            <?php echo $texts['READMORE']; ?>
                                                        </a>
                                                    </p>
